#***************************************************************************************************
# ** 文件名称： dm_ups_user_item_info_gender_info_bayes.sql
# ** 功能描述： 预测男女性别
# ** 创建者： jiahao.dong
# ** 创建日期： 2016-08-26
#***************************************************************************************************
...
FROM(
    SELECT 
        user_id, concat(case when sum(male_prob)>sum(female_prob) then '1' else '2' end, '_2') as sex
    FROM 
        temp.temp_test_sex_probability
    GROUP BY user_id
    UNION ALL
    SELECT 
        user_id, concat(case when sum(case when sex=1 then 1 else 0 end)>sum(case when sex=2 then 1 else 0 end) then '1' else '2' end, '_1') as sex
    FROM 
        dw.dw_usr_address
    WHERE 
