...
drop table temp.temp_mdl_category_user_food_cate_sub1;
create table temp.temp_mdl_category_user_food_cate_sub1 as
    select 
        a.user_id,
        b.category as cate,  
...
drop table temp.temp_mdl_category_user_food_cate_sub2;
create table temp.temp_mdl_category_user_food_cate_sub2 as
    select
        user_id,
        sum(order_cnt) as ord_user_cnt_total
    from
        temp.temp_mdl_category_user_food_cate_sub1
    group by 
        user_id
;
...
drop table temp.temp_mdl_category_user_food_cate;
create table temp.temp_mdl_category_user_food_cate as 
    select
        t1.user_id,
        t1.cate,  
        order_cnt as order_num_total, 
        total_price as total_price_total, 
        last_order_time as last_order_time_max, 
        t2.ord_user_cnt_total as user_month_order_num, 
        '${day}' as record_day
    from
        temp.temp_mdl_category_user_food_cate_sub1 t1
    join
        temp.temp_mdl_category_user_food_cate_sub2 t2
    on(
...
-- sub task 2: 计算cate偏好的分数
drop table temp.temp_rec_user_cate_score;
create table temp.temp_rec_user_cate_score as
    select 
        user_id,
        cate, 
        round((order_num_total/user_month_order_num)*bound_data(total_price_total,2,120)/datediff('${day}',last_order_time_max)+0.01,2) as score
    from 
        temp.temp_mdl_category_user_food_cate
    where 
        cate!='暂无'
;
